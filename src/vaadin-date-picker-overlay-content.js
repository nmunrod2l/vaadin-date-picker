...
`;
  ...
  revealDate(date) {
    if (date) {
      var roundedPosition = Number(this.$.monthScroller.position.toFixed());

      var diff = this._differenceInMonths(date, this._originDate);
      var scrolledAboveViewport = roundedPosition > diff;

      var visibleItems = this.$.monthScroller.clientHeight / this.$.monthScroller.itemHeight;
      var scrolledBelowViewport = roundedPosition + visibleItems - 1 < diff;
